...
describe("isValidTransition", () => {
	...
	test("WaitingForPlayers → Countdown", () => {
		expect(
			isValidTransition(MatchPhase.WaitingForPlayers, MatchPhase.Countdown),
		).toBe(true);
	});

	test("Countdown → Preparing", () => {
		expect(isValidTransition(MatchPhase.Countdown, MatchPhase.Preparing)).toBe(
			true,
		);
	});

	test("Preparing → InProgress", () => {
		expect(isValidTransition(MatchPhase.Preparing, MatchPhase.InProgress)).toBe(
			true,
		);
	});

	test("InProgress → RoundOver", () => {
		expect(isValidTransition(MatchPhase.InProgress, MatchPhase.RoundOver)).toBe(
			true,
		);
	});

	test("RoundOver → Rewarding", () => {
		expect(isValidTransition(MatchPhase.RoundOver, MatchPhase.Rewarding)).toBe(
			true,
		);
	});
	...
	test("Rewarding → WaitingForPlayers", () => {
		expect(
			isValidTransition(MatchPhase.Rewarding, MatchPhase.WaitingForPlayers),
		).toBe(true);
	});
	...
	test("Countdown → WaitingForPlayers (abort)", () => {
		expect(
			isValidTransition(MatchPhase.Countdown, MatchPhase.WaitingForPlayers),
		).toBe(true);
	});

	test("InProgress → WaitingForPlayers (abort)", () => {
		expect(
			isValidTransition(MatchPhase.InProgress, MatchPhase.WaitingForPlayers),
		).toBe(true);
	});
	...
	test("WaitingForPlayers → InProgress is invalid (skips phases)", () => {
		expect(
			isValidTransition(MatchPhase.WaitingForPlayers, MatchPhase.InProgress),
		).toBe(false);
	});

	test("Preparing → WaitingForPlayers is invalid (no abort from Preparing)", () => {
		expect(
			isValidTransition(MatchPhase.Preparing, MatchPhase.WaitingForPlayers),
		).toBe(false);
	});

	test("RoundOver → WaitingForPlayers is invalid (must go through Rewarding)", () => {
		expect(
			isValidTransition(MatchPhase.RoundOver, MatchPhase.WaitingForPlayers),
		).toBe(false);
	});

	test("Rewarding → InProgress is invalid (can only go to WaitingForPlayers)", () => {
		expect(isValidTransition(MatchPhase.Rewarding, MatchPhase.InProgress)).toBe(
			false,
		);
	});
	...
	test("WaitingForPlayers → Rewarding is invalid (skips entire flow)", () => {
		expect(
			isValidTransition(MatchPhase.WaitingForPlayers, MatchPhase.Rewarding),
		).toBe(false);
	});

	test("InProgress → Countdown is invalid (backward)", () => {
		expect(isValidTransition(MatchPhase.InProgress, MatchPhase.Countdown)).toBe(
			false,
		);
	});
});
