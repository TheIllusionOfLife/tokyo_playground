...
| v0.27.0 | 2026-04-03 | Day 30 stability: safeHandler on 17 handlers, BindToClose ordering fix, Fisher-Yates maxPlayers, badge IDs configured, POI rate limit, test infra (CI + bun test), repo hygiene (docs/ move, binary removal, stamp cleanup) |
| v0.28.0 | 2026-04-03 | Day 60 hardening: 66 tests (rewardCalc, matchPhase, proximityUtils), typed microEventProgress, CooldownTracker utility (14 callers), teleport anti-cheat (300-stud threshold, 2 strikes), micro-event analytics, HachiAbilityService extracted from LobbyService |
| v0.29.0 | 2026-04-04 | Day 90 boundary decomposition: network.ts sliced into 4 bounded-context modules, game-store.ts split into 5 action slices, EconomyService + InventoryService extracted from PlayerDataService, round resolution pure functions from MatchService (21 new tests, 87 total), HachiRideMinigame decomposed into 4 helper modules (anticheat, evolution, sky-dragon, collectibles, -40% lines), architecture documentation |
