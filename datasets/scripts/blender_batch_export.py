...
#
# 1. Scales all mesh objects by SCALE factor (baked into vertex positions)
# 2. Exports in batches of BATCH_SIZE into separate FBX files
#
...
SCALE = 18.0  # Scale factor: old city uses ~18 studs per meter
...
# Step 1: Scale all meshes physically (bake into vertex positions)
...
# Scale from world origin so positions scale correctly too
...
print("NOTE: Meshes in Blender are now scaled. Undo (Cmd+Z) or reload the file to restore original scale.")
